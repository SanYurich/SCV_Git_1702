...
Чтобы удалить не нужную ветку, **НО смердженную с другой веткой**

    git branch -d name_branch
*Помни! Всё получиться, если находиться на неудаляемой ветке*

Чтобы насильно удалить ветку

    git branch -D name_branch
>Аккуратней с этой командой!!
...
>Помни! Сделай коммит после решения конфликта
...
3. Создай локальный репозиторий
...
5. Последовательноть работы с Pull request

    1. `Fork` (кнопка) репозитория, с которым будет проводиться работа
    2. `Clone` в локальный репозиторий
    3. Создть новую ветку и проводить в ней изменения
    4. `Push` локального репозитория на удаленный
    5. `Pull` request (кнопка)
